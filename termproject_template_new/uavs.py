...
class uav():
    ...
    def evn_detect(self,pos, time):
        ...
        while(True):
            ...
            #             if(len(M_list[i][j]) != 0):
            #                 if([M_list[i][j][0], M_list[i][j][1][-1]] in list_target):
            #                     pass
            #                 else:
            ...
            for i in range(len(pos)):
                ...
                if(distance(pos[i],self.m.start_pos) >= self.m.map_size[1]/3):
                    ...
                    #     if(distance(pos[i], list_target[j][1]) < 100):
                    #         info.append(['new target', list_target[j], i])


                    # ini buat deteksi kalo deket forbid
                    for f in range(len(Forbid) - 1):
                        for g in range(len(Forbid[f]) - 1):
                            if(distance(pos[i], Forbid[f][g]) <= 40):
                                info.append(['forbid area', i, f, g])

                    leader = 5*int(i/5)
                    for s in range(i+1,leader+5):
                        dis = distance(pos[i],pos[s])

                        # ini buat deteksi kalo hampir tabrakan
                        if(dis <= 50 and dis > 10):
                        	info.append(['collision danger', i, s])
                        ...
                        # lead_range = distance(pos[s], pos[leader])

                        # if(lead_range >= 500 and s not in fail_list):
                        #     info.append(['far', s, leader])
